        ...
        return try {
            when (testConfig.protocolVersion) {
                ...
                MqttProtocolVersion.MQTT_3_1_1 -> {
                    ...
                    val testClient = clientBuilder.buildAsync()
                    val connectBuilder =
                        com.hivemq.client.mqtt.mqtt3.message.connect.Mqtt3Connect
                            .builder()
                            .cleanSession(true)
                            .keepAlive(10)
    ...
    private suspend fun connectMqtt5(config: ConnectionConfig) {
        ...
        if (connAck.reasonCode.isError) {
            _connectionState.value =
                ConnectionState.Error(
                    message =
                        MqttErrorMapper.mapConnectionError(
                            Exception("Broker rejected: ${connAck.reasonCode}"),
                            config.host,
                            config.port,
                        ),
                    cause = null,
        ...
        if (connAck.returnCode.isError) {
            _connectionState.value =
                ConnectionState.Error(
                    message =
                        MqttErrorMapper.mapConnectionError(
                            Exception("Broker rejected: ${connAck.returnCode}"),
                            config.host,
                            config.port,
                        ),
                    cause = null,
        ...
        try {
            mqtt5Client?.let { client ->
                ...
                if (userProperties.isNotEmpty()) {
                    val propsBuilder =
                        com.hivemq.client.mqtt.mqtt5.datatypes.Mqtt5UserProperties
                            .builder()
