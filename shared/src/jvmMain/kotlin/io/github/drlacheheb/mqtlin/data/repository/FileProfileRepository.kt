...
class FileProfileRepository(
    ...
    private val defaultProfiles: List<ConnectionConfig> = listOf(
        ConnectionConfig(
    ...
    override suspend fun exportProfilesJson(includePasswords: Boolean): String = withContext(Dispatchers.IO) {
        mutex.withLock {
            val profiles = readProfilesFromFile().map { profile ->
    ...
    override suspend fun importProfilesJson(jsonContent: String): List<ConnectionConfig> = withContext(Dispatchers.IO) {
        mutex.withLock {
