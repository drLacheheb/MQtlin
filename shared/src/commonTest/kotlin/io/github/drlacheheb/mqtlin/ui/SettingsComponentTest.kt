...
class SettingsComponentTest {
    @Test
    fun `default settings component initial state is under construction and close triggers callback`() =
        runTest {
    ...
    @Test
    fun `opening settings in RootComponent activates dialogSlot without closing active workspace or connection`() =
        runTest {
            val lifecycle = LifecycleRegistry()
            lifecycle.resume()
            val context = DefaultComponentContext(lifecycle = lifecycle)
            ...
            // Initially no dialog
            rootComponent.dialogSlot.value.child
                .shouldBeNull()
            rootComponent.childStack.value.active.instance
                .shouldBeInstanceOf<RootComponent.RootChild.Connection>()
            ...
            rootComponent.dialogSlot.value.child
                ?.instance
                .shouldBeInstanceOf<RootComponent.DialogChild.Settings>()
