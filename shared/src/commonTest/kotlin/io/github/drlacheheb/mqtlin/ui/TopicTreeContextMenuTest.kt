...
class TopicTreeContextMenuTest {
    ...
    @Test
    fun `publish here action selects topic and prepares workspace state`() = runTest {
        ...
        component.state.value.selectedNode?.lastMessage?.payloadString shouldBe "active"
    }

    @Test
    fun `context menu delete retained triggers 0-byte deletion on single topic`() = runTest {
        val testDispatcher = StandardTestDispatcher(testScheduler)
        val fakeRepo = FakeMqttRepository()
        val lifecycle = LifecycleRegistry()
        lifecycle.resume()
    ...
    @Test
    fun `context menu purge branch triggers batch retained purge under selected branch`() = runTest {
        val testDispatcher = StandardTestDispatcher(testScheduler)
        val fakeRepo = FakeMqttRepository()
        val lifecycle = LifecycleRegistry()
        lifecycle.resume()
    ...
    fun `collectAllRetainedLeafPaths accurately filters only retained descendants`() {
        val leaf1 = TopicNode("temp", "home/kitchen/temp", isLeaf = true, lastMessage = MqttMessage("home/kitchen/temp", byteArrayOf(1), isRetained = true))
        val leaf2 = TopicNode("humidity", "home/kitchen/humidity", isLeaf = true, lastMessage = MqttMessage("home/kitchen/humidity", byteArrayOf(2), isRetained = false))
        val leaf3 = TopicNode("light", "home/kitchen/light", isLeaf = true, lastMessage = MqttMessage("home/kitchen/light", byteArrayOf(3), isRetained = true))

        val kitchenNode = TopicNode(
            segment = "kitchen",
